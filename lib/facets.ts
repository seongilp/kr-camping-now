 ...
 * 벌린다. 색맹 안전: 빨강-초록 단독 대비를 피하고 명도 차를 함께 준다(글램핑=앰버, 카라반=바이올렛,
 * 오토캠핑=로즈, 일반야영=초록). **내 위치 점(파랑 #3b82f6)과 겹치지 않게 네 색 모두 파랑 계열을
 * 피한다.** 일반야영(2,292)이 가장 흔하지만 "일반 야영장=초록"이 직관적이라 팀 요청으로 초록을 배정했다.
 */
export const INDUTY_COLOR: Record<string, string> = {
  glamping: '#f59e0b', // 앰버
  caravan: '#a855f7', // 바이올렛
  auto: '#f43f5e', // 로즈(옛 에메랄드는 일반야영 초록과 겹쳐 이동)
  general: '#22c55e', // 초록
};
